...
data<-read.csv("references_pollens_fluo_all.csv", h=T)
names(data)[1]<-"Cytometry_Name_pollens"
## charger le tableau qui contient le nom des échantillons et 
names<-read.csv("C:/Users/User/OneDrive - UQAM/PaqLab/Sarah/CytoR/data_reference/Collection_Reference_Pollens.csv", sep= ";",h=T)
library(dplyr)
...
write.csv(datamod, 'trainingdata_genus_fluo.csv', row.names = F) ## jeux de données complet
write.csv(trainset, 'trainset_genus_fluo.csv', row.names = F) ## jeux de données pour entraîner le modèle
write.csv(testset, 'testset_genus_fluo.csv', row.names = F) ## jeux de données pour tester le modèle
